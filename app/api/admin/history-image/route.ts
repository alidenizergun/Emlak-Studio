...
export async function GET(request: NextRequest) {
  ...
  try {
    const userId = normalizePhone(request.nextUrl.searchParams.get('userId'));
    ...
    if (!userId || !entryId.includes(':')) {
      return NextResponse.json({ success: false, error: 'Geçersiz görsel isteği.' }, { status: 400 });
    }
