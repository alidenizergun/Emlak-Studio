...
function buildConstraintBlock(
    ...
    return [
        'ARCHITECTURE RULES:',
        formatBulletLines([
            'Never change room dimensions, wall positions, ceiling geometry, openings, or camera perspective.',
            'Never invent new walls, partitions, columns, vertical planes, or fake depth surfaces.',
            'Keep floor pattern direction, corner joins, and architectural anchors identical to the input.',
        ]),
        '',
        'ROOM PLAN:',
        formatBulletLines([
            roomPlan.useCase,
            `Focal point: ${roomPlan.focalPoint}`,
            `Must have: ${roomPlan.mustHave}`,
            `Avoid: ${roomPlan.avoid}`,
            `Circulation: ${roomPlan.circulation}`,
            `Scale: ${roomPlan.scale}`,
        ]),
        '',
        'STYLE PLAN:',
        formatBulletLines([
            stylePlan.summary,
            `Palette/materials: ${stylePlan.palette} ${stylePlan.materials}`,
            `Lighting/silhouette: ${stylePlan.lighting} ${stylePlan.silhouette}`,
            `Accessories/textiles: ${stylePlan.accessories} ${stylePlan.textiles}`,
        ...
        formatBulletLines([
            comboPlan,
            `Intensity: ${intensityPlan.label}. ${intensityPlan.furnitureDensity}`,
        ...
        'NEGATIVE RULES:',
        formatBulletLines([
            'Do not overfill the room or block doors, windows, radiators, or key openings.',
            'Do not create collage, split-scene, partial-room, or panel-like outputs.',
            'Do not leave semi-transparent objects, duplicate outlines, seam bands, or cut furniture.',
            'If a fixture is restyled, keep its mounting point fixed.',
            'Room function wins over style if they conflict.',
            watermarkRule,
            cropRule,
            cleanupRule,
            ghostRule,
...
export function generateStagePrompt(input: StagePromptInput): string {
    ...
    return [
        ...
        '',
        'FINAL OUTPUT RULE:',
        '- Return one unified, listing-ready furnished image with clear room function, readable style, premium cleanliness, and unchanged architecture.',
    ]
        .filter(Boolean)
        .join('\n');
}
